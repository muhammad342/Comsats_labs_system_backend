...
const allRequestAprrovedByDco = asyncHandler(async (req, res) => {
  const request = await Request.find({ dcoApproved: true });
...
const allRequestRejectedByDcoReport = asyncHandler(async (req, res) => {
  const request = await Request.find({ dcoApproved: false });
  if (request) {
    res.json(request);
  } else {
    res.status(404);
...
const allRequestAprrovedByCommitteeReport = asyncHandler(async (req, res) => {
...
const allRequestRejectedByCommitteeReport = asyncHandler(async (req, res) => {
  const request = await Request.find({
...
const allRequestAprrovedByWorksReport = asyncHandler(async (req, res) => {
  const request = await Request.find({
    committeApproved: true,
    dcoApproved: true,
    worksApproved: true,
...
const allRequestRejectedByWorksReport = asyncHandler(async (req, res) => {
  const request = await Request.find({
    committeApproved: true,
    dcoApproved: true,
    worksApproved: false,
...
const allRequestAprrovedByNOCReport = asyncHandler(async (req, res) => {
  const request = await Request.find({
    committeApproved: true,
    dcoApproved: true,
...
const allRequestRejectedByNOCReport = asyncHandler(async (req, res) => {
  const request = await Request.find({
    committeApproved: true,
    dcoApproved: true,
    nocApproved: false,
...
const addFacultyRequest = asyncHandler(async (req, res) => {
  const { user, name, courseCode, courseTitle, numberOfStudent, lab, type } =
    req.body;
  let request = await FacultyRequest.create({
    user,
    name,
    courseCode,
    courseTitle,
...
module.exports = {
  ...
  allRequestRejectedByWorksReport,
  ...
  approvedByNoc,
  rejectedByNoc,
  allRequestAprrovedByNOC,
  RequestById,
  RequestDeadline,
  requestCompleted,
  acceptedAllRequest,
  acceptedSoftwareAllRequest,
  acceptedHardwareAllRequest,
  deleteRequest,
};
